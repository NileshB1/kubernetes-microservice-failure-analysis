...
def get_db_connection():
    ...
    return psycopg2.connect(host=os.getenv("POSTGRES_HOST", "postgres"),
        port=int(os.getenv("POSTGRES_PORT","5432")), user=os.getenv("POSTGRES_USER", "sparkuser"),
        password=os.getenv("POSTGRES_PASSWORD","sparkpass"),  dbname=os.getenv("POSTGRES_DB", "microservice_analysis"))
...
def setup_plot_style():
    ...
    plt.rcParams.update({"figure.dpi": 150, "font.size": 12,
            "axes.titlesize": 16, "axes.labelsize": 13, "figure.figsize": (12, 8)})
...
def plot_propagation_heatmap(output_dir: str = "/output") -> str:
    ...
    try:
        query = """
            SELECT caller_service, callee_service, propagation_score  FROM cross_service_pairs
            WHERE propagation_score > 0  ORDER BY propagation_score DESC
        """
    ...
    pivot = df.pivot_table(
        ...
        values="propagation_score",  aggfunc="mean",
        fill_value=0,
    )
    ...
    sns.heatmap(pivot,  annot=True,
        fmt=".2f", cmap="YlOrRd",
        cbar_kws={"label": "Propagation Score"}, ax=ax, linewidths=0.5,
    )
...
def plot_anomaly_timeseries(output_dir: str = "/output") -> str:
    ...
    try:
        ...
        top_services = pd.read_sql(
            """ SELECT service_name, COUNT(*) AS anomaly_count   FROM anomaly_scores
            WHERE is_anomaly_overall = 1  GROUP BY service_name
            ORDER BY anomaly_count DESC     LIMIT 4
        """,  conn )
        ...
        anomaly_data = pd.read_sql(
            ...
            FROM anomaly_scores WHERE service_name IN ({placeholders}) ORDER BY time_bucket
        """, conn)
    finally:
        conn.close()
...
def plot_failure_pattern_distribution(output_dir: str = "/output") -> str:
    ...
    logger.info(f"Plotting rq2: Failure pattern distribution...,")
    setup_plot_style()
    ...
    try:
        df = pd.read_sql(
            ...
            FROM failure_patterns  GROUP BY pattern_type  ORDER BY total_occurrences DESC
        """, conn)
    finally:
        conn.close()
    ...
    for bar, val in zip(bars, df["total_occurrences"], strict=False):
...
def plot_scaling_curves(output_dir: str = "/output") -> str:
    """
    Plot execution time vs data size and throughput curves...
    """
    ...
    try:
        df = pd.read_sql(
            ...
            FROM scalability_metrics  ORDER BY data_size
        """, conn)
    finally:
        conn.close()
    ...
    # Aggregate
    agg = (
        df.groupby("data_size")
        .agg(
            {"total_sec": ["mean", "std"], "throughput_rows_per_sec": "mean"}
        ).reset_index()
    )
    ...
    ax1.errorbar(
        agg["data_size"], agg["avg_time"],
        yerr=agg["std_time"],fmt="o-",
        capsize=5,  markersize=8, linewidth=2.5, color="#2c7bb6",
    )
    ax1.set_xscale("log")
    ax1.set_yscale("log")
    ...
    ax2.plot(agg["data_size"], agg["avg_throughput"], "o-", markersize=8, linewidth=2.5, color="#d7191c")
    ax2.set_xscale("log")
...
def plot_scalability_efficiency(output_dir: str = "/output") -> str:
    ...
    logger.info(f"Plotting RQ3: Scalability efficiency....")
    setup_plot_style()
    ...
    ax1.plot(data_ratios, speedups, "o-", color="#2c7bb6", linewidth=2.5, markersize=10, label="Actual Speed-up")
    ax1.plot(data_ratios, data_ratios, "--", color="gray", linewidth=1.5, label="Ideal Linear Speed-up")
    ...
    ax2 = ax1.twinx()
    ax2.plot(data_ratios, efficiencies, "s--", color="#d7191c", linewidth=2, markersize=10, label="Efficiency")
...
def generate_dashboard(output_dir: str = "/output") -> str:
    ...
    try:
        ...
        prop_df = pd.read_sql("""SELECT caller_service, callee_service, propagation_score
            FROM cross_service_pairs ORDER BY propagation_score DESC LIMIT 5""",
            conn)

        # RQ2=> Anomaly summary
        anomaly_summary = pd.read_sql(
            """SELECT COUNT(*) AS total_buckets,
                SUM(is_anomaly_overall) AS total_anomalies FROM anomaly_scores""", conn)
    ...
    if not prop_df.empty:
        ax1.barh(
            ...
            prop_df["propagation_score"], color=sns.color_palette("YlOrRd", len(prop_df)),
        )
    ...
    if not anomaly_summary.empty:
        ...
        ax2.pie(
            [normal, anomalies], labels=["Normal", "Anomalous"],
    ...
    if not scale_summary.empty:
        ax3.plot(scale_summary["data_size"], scale_summary["avg_time_sec"], "o-", linewidth=2.5, markersize=8)
        ax3.set_xscale("log")
        ax3.set_ylabel("Time (s)")
        ax3.set_title("RQ3: Execution Time Scaling")

    #Ssubplot 4: Failure Patterns
    ax4 = fig.add_subplot(2, 3, 4)
    try:
        conn = get_db_connection()
        pattern_df = pd.read_sql(
            """ SELECT pattern_type, SUM(occurrence_count) AS total
            FROM failure_patterns  GROUP BY pattern_type ORDER BY total DESC """, conn)
        conn.close()
    ...
    if not scale_summary.empty:
        ...
        metrics_text += f"Speed-up: {speedup:.2f}x\n\n"
    if not anomaly_summary.empty:
...
def _save_empty_plot(name: str, message: str, output_dir: str) -> str:
    ...
    ax.text(0.5, 0.5, f"{message}\n({name})", ha="center", va="center", fontsize=14, transform=ax.transAxes)
...
#main
def run_visualization(output_dir: str = "/output"):
    """
    Generate all plots for the three research questions
    """
    logger.info("=" * 45)
    logger.info("MODULE 6: RESULTS and VISUALIZATION .....")
    logger.info("=" * 55)
