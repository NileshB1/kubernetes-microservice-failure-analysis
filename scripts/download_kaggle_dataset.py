...
# ============================================================
# Downloads the Microservices Bottleneck Detection Dataset
# from Kaggle and validates that the CSV schemas match what
# the preprocessing module expects.
#
# Usage:
#   python scripts/download_kaggle_dataset.py              # download + validate
#   python scripts/download_kaggle_dataset.py --validate-only  # validate existing
#   python scripts/download_kaggle_dataset.py --data-dir ./data  # custom dir
#
# Prerequisites (choose one):
#   Option A — kagglehub (recommended, no API key needed):
#       pip install kagglehub
#
#   Option B — Kaggle API (requires kaggle.json):
...
#
# If neither is available, the script prints manual download
...
EXPECTED_SCHEMAS: dict[str, dict] = {
    ...
    "trace_response_times.csv": {
        "columns": [
            "span_id", "response_time_ms", "wait_time_ms",
    ...
    "resource_usage.csv": {
        "columns": [
            "pod_id", "timestamp", "cpu_usage_mcores",
            "memory_usage_mb", "network_rx_bytes",
    ...
    "status_codes.csv": {
        "columns": [
            "span_id", "status_code", "error_message", "is_error",
        ],
        "key_column": "span_id",
    },
}
...
def download_via_kaggle_api(data_dir: str) -> bool:
    ...
    if not os.path.isfile(kaggle_json):
        print("  [ERR] kaggle CLI found but no ~/.kaggle/kaggle.json")
