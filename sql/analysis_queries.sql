...
SELECT
    ...
    ROUND(AVG(response_time_ms), 3) AS mean_latency_ms, ROUND(PERCENTILE_APPROX(response_time_ms, 0.50), 3) AS p50_latency_ms,
    ROUND(PERCENTILE_APPROX(response_time_ms, 0.95), 3) AS p95_latency_ms,  ROUND(PERCENTILE_APPROX(response_time_ms, 0.99), 3) AS p99_latency_ms,
    ROUND(MAX(response_time_ms), 3)  AS max_latency_ms, SUM(is_failure) AS failed_spans,
    ROUND(100.0 * SUM(is_failure) / COUNT(*), 3) AS failure_rate_pct FROM telemetry GROUP BY service_name ORDER BY failure_rate_pct DESC

-- The slowest operations, restricted to those with enough traffic to be
-- meaningful
SELECT
    service_name, endpoint,   COUNT(*) AS calls,
    ROUND(AVG(response_time_ms), 3) AS mean_latency_ms,
    ROUND(PERCENTILE_APPROX(response_time_ms, 0.95), 3) AS p95_latency_ms,
    SUM(is_failure) AS failed_calls,    ROUND(100.0 * SUM(is_failure) / COUNT(*), 3)  AS failure_rate_pct
FROM telemetry GROUP BY service_name, endpoint HAVING COUNT(*) >= 100
ORDER BY p95_latency_ms DESC LIMIT 50


-- How deep call chains run
SELECT
    span_count_bucket,
...
-- Ranks services by failure rate using a window function
SELECT
...
-- failure rate per service per hour
SELECT
...
FROM (
    SELECT
        ...
        COUNT(*) AS spans,  SUM(is_failure) AS failed,
        ROUND(100.0 * SUM(is_failure) / COUNT(*), 3) AS failure_rate_pct
...
SELECT
    ...
    SUM(m.spans_in_window) AS spans_observed,  SUM(m.failures_in_window) AS failures_observed,
...
LEFT JOIN (
    SELECT
        ...
        COUNT(*) AS spans_in_window,  SUM(is_failure) AS failures_in_window
    FROM telemetry WHERE start_time_ts IS NOT NULL
    GROUP BY service_name, UNIX_TIMESTAMP(start_time_ts)
) m
  ON  m.service_name = g.inject_service
  -- The Nezha injections run for roughly five minutes from the recorded timestamp
