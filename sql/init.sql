...
CREATE TABLE IF NOT EXISTS processed_telemetry (
    ...
    namespace VARCHAR(64), pod_id  VARCHAR(64), node_id VARCHAR(64),
    response_time_ms DOUBLE PRECISION, wait_time_ms  DOUBLE PRECISION,
    processing_time_ms DOUBLE PRECISION, network_latency_ms DOUBLE PRECISION,
    start_time_ts  TIMESTAMP, end_time_ts  TIMESTAMP,
    duration_ms DOUBLE PRECISION, http_method VARCHAR(16),
    endpoint  VARCHAR(256), status_code INTEGER,
    error_message  TEXT,  is_failure  INTEGER,
    is_latency_spike INTEGER, latency_bucket VARCHAR(16),
    error_category  VARCHAR(32), hour_of_day INTEGER,
...
CREATE TABLE IF NOT EXISTS cross_service_pairs (
    id  SERIAL PRIMARY KEY, caller_service  VARCHAR(128),
    callee_service VARCHAR(128), call_count BIGINT,
    caller_error_count BIGINT, callee_error_count BIGINT,
    co_failure_count  BIGINT, avg_callee_latency_ms DOUBLE PRECISION,
    propagation_score DOUBLE PRECISION, analyzed_at TIMESTAMP DEFAULT NOW()
);

-- RQ1: Propagation chains
CREATE TABLE IF NOT EXISTS propagation_chains (
    id SERIAL PRIMARY KEY, trace_id VARCHAR(64),
    source_service  VARCHAR(128), target_service VARCHAR(128),
    source_timestamp TIMESTAMP,  target_timestamp  TIMESTAMP,
    propagation_lag_sec DOUBLE PRECISION, propagation_depth INTEGER,
...
CREATE TABLE IF NOT EXISTS anomaly_scores (
    ...
    -- Which signal produced the flag: a breach of an absolute SLO ceiling
    -- (persistently degraded) as opposed to a z-score deviation (regressing).
    is_error_rate_slo_breach INTEGER DEFAULT 0, is_latency_slo_breach INTEGER DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS scalability_metrics (
    id  SERIAL PRIMARY KEY,
    label VARCHAR(128), input_rows BIGINT,
    groupby_agg_sec DOUBLE PRECISION, window_fn_sec DOUBLE PRECISION,
    join_sec DOUBLE PRECISION, shuffle_sec DOUBLE PRECISION,
    total_sec DOUBLE PRECISION, data_size BIGINT,
    repetition INTEGER, throughput_rows_per_sec DOUBLE PRECISION,
